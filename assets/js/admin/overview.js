(() => {
      ...
      afterDatasetsDraw(chart) {
        ...
        chart.data.datasets.forEach((dataset, datasetIndex) => {
          ...
          meta.data.forEach((element, index) => {
            ...
            if (isLine) {
              const pointRadius = Number(dataset.pointRadius) || 5;
  ...
  function renderOverviewKPIs(tickets, hitlCases, workOrders) {
    ...
    const completedCases = tickets.filter(isCompleted).length;
    const hitlRequired = tickets.filter(isHitlRequired).length;
    const automatedCases = Math.max(0, totalTickets - hitlRequired);
    ...
    const activeOrders = Math.max(0, totalTickets - completedCases);
    ...
    const automationRate = totalTickets ? ((automatedCases / totalTickets) * 100).toFixed(1) : '0.0';

    setText('totalComplaint7', formatNumber(totalTickets));
    setText('actionWorkOrder', formatNumber(activeOrders));
    setText('completedCases7', formatNumber(completedCases));
    setText('aiAutomationRate', `${automationRate}%`);
    setText('aiAutomationText', `${formatNumber(automatedCases)} / ${formatNumber(totalTickets)} tickets`);
  ...
  function buildSevenDayCohorts(tickets) {
    const referenceDate = startOfDay(getReferenceDate(tickets));
  ...
  function renderComplaintCompletedTrend(tickets) {
    ...
    // Historical event counts must not change when a ticket moves to another status.
    // A complaint is always counted on its intake date, while a completion is
    // counted on the date the technician actually completed the case.
    ...
    const valueLabels = plainValueLabelPlugin('overviewValueLabels');
    ...
    new Chart(canvas, {
      ...
      options: {
        ...
        layout: { padding: { top: 18, right: 26, bottom: 0, left: 10 } },
        ...
        onClick(event, elements, chart) {
          // Drill down only when the user clicks an actual data-point circle.
          const points = chart.getElementsAtEventForMode(
            event,
            'nearest',
            { intersect: true, axis: 'xy' },
        ...
        plugins: {
          legend: { position: 'bottom', labels: { padding: 6, usePointStyle: true, pointStyle: 'line' } },
  ...
  function renderTechnicianCapacity(technicians) {
    ...
    const centerLabel = {
      ...
      afterDraw(chart) {
        ...
        ctx.fillText(`${capacityUsed.toFixed(0)}%`, x, y - 24);
        ctx.font = '700 12px Arial';
    ...
    new Chart(canvas, {
      ...
      options: {
        ...
        onClick(event, elements, chart) {
          ...
          window.location.href = `../smart/asset_detail.php?asset=${encodeURIComponent(asset)}`;
        },
        layout: { padding: { top: 34, right: 34, left: 8, bottom: 8 } },
  ...
  function renderTrendCasesDetail(tickets) {
    ...
    const selectedDate = dateValue ? parseDate(`${dateValue}T00:00:00`) : startOfDay(getReferenceDate(tickets));
    ...
    const openFromSelectedIntake = receivedCases.filter(ticket => !isCompleted(ticket));
    ...
    setText('detailOpenCases', formatNumber(openFromSelectedIntake.length));
