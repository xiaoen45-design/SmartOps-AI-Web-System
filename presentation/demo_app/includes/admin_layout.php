...
function admin_ui_header(string $title, string $active = '', string $subtitle = '', bool $detailMode = false, ?string $detailBackPath = null): void {
    ...
    $bodyClass = trim('admin-dashboard-page ' . $sharedShellClass . ($detailMode ? 'admin-detail-page ' : '') . ($active !== '' ? $active . '-dashboard-body ' : '') . ($active === 'hitl' ? 'hitl-dashboard-page' : '') . ' ' . ($active === 'workforce' ? 'workforce-dashboard-body' : '') . ' ' . ($active === 'technicians' ? 'technician-management-body' : ''));
    ?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <link rel="stylesheet" href="<?= e(app_url('assets/css/admin/responsive.css')) ?>?v=20260729-workforce-history-simple-v3">
  <?php if (in_array($active, ['overview', 'smart'], true) && !$detailMode): ?>
  <link rel="stylesheet" href="<?= e(app_url('assets/css/admin/overview_onepage.css')) ?>?v=<?= e(substr((string)@md5_file(dirname(__DIR__) . '/assets/css/admin/overview_onepage.css'), 0, 12)) ?>">
  <?php endif; ?>
  ...
  <link rel="stylesheet" href="<?= e(app_url('assets/css/admin/dashboard_shared.css')) ?>?v=<?= e(substr((string)@md5_file(dirname(__DIR__) . '/assets/css/admin/dashboard_shared.css'), 0, 12)) ?>">
  <?php endif; ?>
  <?php if (!$detailMode && in_array($active, ['overview', 'smart', 'hitl', 'workforce'], true)): ?>
  <link rel="stylesheet" href="<?= e(app_url('assets/css/admin/dashboard_stable.css')) ?>?v=<?= e(substr((string)@md5_file(dirname(__DIR__) . '/assets/css/admin/dashboard_stable.css'), 0, 12)) ?>">
  <?php endif; ?>
</head>
...
function admin_ui_footer(array $vendorScripts = []): void {
  ...
  <?php if (!empty($_GET['demo_open_notifications'])): ?>
  <script>window.addEventListener('load',function(){setTimeout(function(){var b=document.querySelector('[data-notification-toggle]');if(b)b.click();},350);});</script>
  <?php endif; ?>
