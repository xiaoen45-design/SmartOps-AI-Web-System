...
$classRestricted=$trigger==='Out of Knowledge Base';
$guidanceRestricted=hitl_is_low_confidence_case($hitl) || $classRestricted;
if ($classRestricted) { $severity=''; $priority=''; }
if ($classRestricted) { $hotelAsset='Other'; $component=''; }
...
try {
    ...
    $pdo->commit();
    $_SESSION['smartops_demo_last_assignment'] = [
        'case_id' => $caseId,
...
if (defined('SMARTOPS_DEMO_RUNTIME') && SMARTOPS_DEMO_RUNTIME) {
    ...
    $back .= $separator
        . 'demo_assigned=1'
        . '&demo_case_id=' . rawurlencode($caseId)
        . '&demo_technician_id=' . rawurlencode($technicianId);
}
redirect_to($back);
