...
$rows = fetch_all("SELECT h.*, s.ai_confidence
                   ...
                   ORDER BY CASE WHEN h.room='Room 305' THEN 0 ELSE 1 END, h.id DESC");
  ...
  <div class="table-box pending-review-table-box">
    <table class="dashboard-table" id="pendingReviewTable">
      ...
      <?php foreach ($rows as $row):
          ...
          $isDemoRoom305 = strcasecmp(trim((string)($row['room'] ?? '')), 'Room 305') === 0;
      ?>
        <tr class="<?= $isDemoRoom305 ? 'demo-room305-row' : '' ?>" data-demo-room305="<?= $isDemoRoom305 ? '1' : '0' ?>" data-case-types="<?= e(implode(' ', $flags)) ?>">
          <td><?= e($caseId) ?><?php if ($isDemoRoom305): ?><span class="demo-room305-chip">GUIDED DEMO</span><?php endif; ?></td><td><?= e($row['room'] ?: '-') ?></td>
          ...
          <td><button type="button" class="review-btn<?= $isDemoRoom305 ? ' demo-room305-review' : '' ?>" data-review='<?= e(json_encode($detail, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)) ?>'>Review</button></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
...
.demo-room305-row td:first-child {
  font-weight: 800;
  color: #1d4ed8;
}
.demo-room305-chip {
...
.demo-room305-review {
  animation: demoRoom305Pulse 1.35s ease-in-out infinite;
}
@keyframes demoRoom305Pulse {
  50% { box-shadow: 0 0 0 7px rgba(37,99,235,.15); }
}
@media (prefers-reduced-motion: reduce) {
  .demo-room305-review { animation: none; }
}
</style>
<script>
document.addEventListener('DOMContentLoaded', function () {
  var params = new URLSearchParams(window.location.search);
  if (params.get('demo_assigned') === '1') {
    var assignedCaseId = params.get('demo_case_id') || '';
    var assignedTechnicianId = params.get('demo_technician_id') || '';
    window.parent.postMessage({
      type: 'SMARTOPS_DEMO_ASSIGNED',
      caseId: assignedCaseId,
      technicianId: assignedTechnicianId
    }, window.location.origin);
  }

  var row = document.querySelector('[data-demo-room305="1"]');
  if (row) {
    window.setTimeout(function () { row.scrollIntoView({ block: 'center', behavior: 'smooth' }); }, 250);
  }
  var review = document.querySelector('.demo-room305-review');
  if (review) {
    review.addEventListener('click', function () {
      var caseId = '';
      try {
        caseId = JSON.parse(review.getAttribute('data-review') || '{}').caseId || '';
      } catch (error) {
        caseId = '';
      }
      window.parent.postMessage({ type: 'SMARTOPS_DEMO_REVIEW_OPENED', caseId: caseId }, window.location.origin);
    });
  }
});
</script>
